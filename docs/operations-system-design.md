...
`PHASE_ORDER` 由运营维护——不同副本的阶段数量不同，运营知道当前副本有多少个阶段。这个数组同时决定了 phase 校验的白名单。
...
当开发需要修改 `data.js` 的结构（新增字段、修改类型），必须遵守**向后兼容原则**，分三步走：

```
Step 1: 新字段标记为 optional，发布 feature PR → 合并 main
Step 2: UI 代码同时支持有该字段和无该字段两种情况
Step 3: 在后续 PR 中将字段改为 required（此时所有 content/* 分支
        自然已通过后续更新携带该字段）
```
...
```
1. 通知运营者暂停数据更新
2. 开发者合并 schema change → main
3. Agent 立刻更新 data.js 适配新结构
4. 恢复运营轨
```
...
每届新赛事开始时，运营侧需要执行一次批量初始化操作，将 `data.js` 从上届结束状态迁移到新赛事就绪状态：
...
| `PHASE_ORDER` | 调整 | 切换为当前副本的阶段序列 |

**执行方式：** 此项操作工作量大、涉及全量数据替换，不走常规的 `content/*` PR 流程。由运营提供新赛事信息给开发者，开发者创建 `feature/init-season-<edition>` 分支，手工编写初始化后的 `data.js`，走 Code Review + PR 合入 main。

> 此流程在未来可迭代为 Agent 自动执行——运营提供一份赛事初始化表单，Agent 按模板生成完整的 `data.js`。初期手工执行确保质量，积累经验后再自动化。
...
| 编辑 HTML / CSS / JS（非 data.js） | ✅ | ❌ | CODEOWNERS + Agent |
| 编辑 `data.js` 值 | ❌ | ✅ | CODEOWNERS + CI |
| 编辑 `data.js` 结构 | ✅ | ❌ | CODEOWNERS |
...
CI（GitHub Actions）在每次 PR 时运行，作为独立于 Agent 的第二道机器防线。校验覆盖两类 PR：

**所有 PR 通用校验：**

| 类别 | 规则 |
|------|------|
| **语法** | `data.js` 可解析，无语法错误 |
| **结构** | `RACE_DATA` 包含全部六个顶层 key（meta / teams / news / broadcasters / notices / sponsors） |
| **排名** | `rank` 无重复、无跳号、无越界 |
| **阶段** | `phase` 值在 `PHASE_ORDER` 白名单内 |
| **血量** | `bossHP` ∈ [0, 100] |
| **区域** | `region` ∈ `{JP, NA, EU, OC, CN, KR}` |
...
| 类别 | 规则 |
|------|------|
| **文件范围** | diff 仅包含 `data.js`。若包含其他文件 → FAIL，输出违规文件列表，由 Agent 读取日志修正 |

这一步确保运营侧的 PR 在物理层面无法携带页面代码变更。与 CODEOWNERS 的分工：CODEOWNERS 在审批层面拦截（要求 dev-team review），CI 文件范围检查在自动化层面拦截（直接 FAIL，不给合入机会）。两者共存形成纵深防御——CI 是第一道自动门禁，CODEOWNERS 是第二道审批兜底。
...
├── .github/
│   └── workflows/
│       └── validate.yml         # CI：schema 校验 + 文件范围检查
│
├── index.html                   # 启动页 / 排名总览
...
| 阶段 | 内容 | 依赖 |
|------|------|------|
| **阶段 1** | 静态前端页面 + `data.js` 数据层 | 无 |
| **阶段 2** | Skills + Rules + Prompt Templates 正式化 | 阶段 1 完成 |
| **阶段 3** | CI 校验层（GitHub Actions）+ Branch Protection 落地 | 阶段 2 完成 |
| **阶段 4** | Extensions（合并门禁 + 预览链接）+ CODEOWNERS | 阶段 3 完成 |
| **阶段 5** | schema/ 目录 → CI 以 Schema 驱动校验 | 阶段 4 完成 |
